...
export default function Game(){
  ...
  useEffect(()=>{
    ...
    const loop=()=>{
      ...
      for(const e of enemiesRef.current){
        const d=ensureData(e); const k=asEnemyKind(text(d.kind,'jelly')); d.t=num(d.t,0)+0.03*ts
        if(k==='jelly'){ e.x+=num(e.vx,0)*ts*0.75; e.y+=Math.sin(num(d.t)*2.0)*2.6 }
        else if(k==='squid'){ e.x+=num(e.vx,0)*ts*1.2; e.y+=Math.sin(num(d.t)*3.0)*1.8 }
        else if(k==='manta'){ e.x+=num(e.vx,0)*ts*1.05; e.y+=Math.sin(num(d.t)*1.2)*3.2 }
        else if(k==='nautilus'){ e.x+=num(e.vx,0)*ts*0.9; if(frame.current%46===0){
        ...
        else if(k==='puffer'){ e.x+=num(e.vx,0)*ts; d.scale=1+Math.sin(num(d.t)*3)*0.25 }
        else if(k==='crab'){ e.x+=num(e.vx,0)*ts*1.15; e.y+=Math.sin(num(d.t)*2.6)*1.2 }
      }
      enemiesRef.current=enemiesRef.current.filter(e=>e.x>-160)
      ...
      for(const e of enemiesRef.current) if(flag(e.data?.bullet)) e.y+=num(e.vy,0)*ts
  ...
  function draw(ctx:CanvasRenderingContext2D){
    ...
    for(const e of enemiesRef.current){
      ...
      if(flag((e as any).data?.bullet)){
        ...
        if(kind==='spear'){
          ...
          ctx.fillStyle='rgba(239,68,68,.9)'; ctx.beginPath(); ctx.arc(0,0,10,0,Math.PI*2); ctx.fill()
        }
      } else {
        ...
        } else if(k==='squid'){ squidShape(ctx,(e as any).w,(e as any).h); ctx.fill(); ctx.stroke(); ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(0,-10,3,0,Math.PI*2); ctx.fill() }
        else if(k==='manta'){ mantaShape(ctx,(e as any).w,(e as any).h); ctx.fill(); ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(-10,-6,3,0,Math.PI*2); ctx.arc(10,-6,3,0,Math.PI*2); ctx.fill() }
        else if(k==='nautilus'){ nautilusShape(ctx,(e as any).w); ctx.fill(); ctx.stroke(); ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(6,-4,3,0,Math.PI*2); ctx.fill() }
        else if(k==='puffer'){ const sc=num((e as any).data?.scale,1); pufferShape(ctx,(e as any).w*sc,(e as any).h*sc); ctx.fill(); ctx.stroke()
          ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(-5,-2,2.5,0,Math.PI*2); ctx.arc(5,-2,2.5,0,Math.PI*2); ctx.fill()
        } else if(k==='crab'){ crabShape(ctx,(e as any).w,(e as any).h); ctx.fill(); ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(-8,-4,2.5,0,Math.PI*2); ctx.arc(8,-4,2.5,0,Math.PI*2); ctx.fill() }
      }
      ctx.restore()
    }
    ...
    // boss + HP bar
    if(bossRef.current){
      const b=bossRef.current, aura=num(b.data?.aura,1)
      ctx.save(); ctx.translate(b.x,b.y)
      const halo=ctx.createRadialGradient(0,0,0,0,0,b.w/1.3)
      halo.addColorStop(0,`rgba(167,139,250,${0.7*aura})`)
      halo.addColorStop(1,'rgba(167,139,250,0)')
      ctx.fillStyle=halo; ctx.beginPath(); ctx.arc(0,0,b.w/1.35,0,Math.PI*2); ctx.fill()

      ctx.fillStyle='#a78bfa'; roundedRect(ctx,-b.w/2,-b.h/2,b.w,b.h,20); ctx.fill()
      ctx.fillStyle='rgba(250,204,21,.35)'
      ctx.beginPath(); ctx.moveTo(-b.w*0.65,-10); ctx.quadraticCurveTo(-b.w*0.95,-b.h*0.6,-b.w*0.25,-b.h*0.1); ctx.quadraticCurveTo(-b.w*0.95,b.h*0.6,-b.w*0.65,10); ctx.fill()
      ctx.beginPath(); ctx.moveTo(b.w*0.65,-10); ctx.quadraticCurveTo(b.w*0.95,-b.h*0.6,b.w*0.25,-b.h*0.1); ctx.quadraticCurveTo(b.w*0.95,b.h*0.6,b.w*0.65,10); ctx.fill()
      ctx.fillStyle='#0b1220'; ctx.beginPath(); ctx.arc(-16,-8,4,0,Math.PI*2); ctx.arc(16,-8,4,0,Math.PI*2); ctx.fill()
      ctx.strokeStyle='#fef08a'; ctx.beginPath(); ctx.moveTo(-22,-b.h*0.28); ctx.lineTo(-10,-b.h*0.5); ctx.moveTo(22,-b.h*0.28); ctx.lineTo(10,-b.h*0.5); ctx.stroke()
      ctx.restore()

      const hp = Math.max(0, num(bossRef.current.hp,0))
      const barW = 180, barH = 10, left = bossRef.current.x - barW/2, top = bossRef.current.y - bossRef.current.h/2 - 18
      ctx.fillStyle='rgba(0,0,0,.35)'; roundedRect(ctx,left, top, barW, barH, 6); ctx.fill()
      const pct = Math.max(0, Math.min(1, hp / (level.current.bossHp+80)))
      const grd=ctx.createLinearGradient(left,top,left+barW,top)
      grd.addColorStop(0,'#fda4af'); grd.addColorStop(1,'#a78bfa')
      roundedRect(ctx,left, top, barW*pct, barH, 6); ctx.fillStyle=grd; ctx.fill()
    }

    // player bullets
    for(const b of bulletsRef.current){
      const kind=asWeaponId(text(b.data?.kind,'blaster')); const c=WEAPONS[kind]?.color ?? '#f8fafc'
      ctx.fillStyle=c; ctx.fillRect(Math.floor((b as any).x-(b as any).w/2),Math.floor((b as any).y-(b as any).h/2),(b as any).w,(b as any).h)
    }
...
/>
      ...
      <canvas className="canvas" ref={canvasRef}
        onPointerDown={()=>{ if(showHelp) setShowHelp(false); else if(gameOver) softReset() }} />
    </div>
  )
}
