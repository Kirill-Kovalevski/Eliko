/* Game.tsx — desktop: arrows+space; phone (iPhone 12 Pro+): left/right thumbs; tap-to-move on left */
...
function detectMode(): "touch" | "desktop" {
  ...
  // iPhone 12 Pro and up: logical width <= 430px and height >= 780px
  const phoneSized =
    window.innerWidth <= 430 && Math.max(window.innerWidth, window.innerHeight) >= 780;
...
const asWeaponId = (x: unknown): WeaponId =>
  (WEAPON_TIER as readonly string[]).includes(String(x)) ? (x as WeaponId) : ("blaster" as WeaponId);
...
function explode(x: number, y: number, c: string, n = 16) {
  const out: Entity[] = [];
  for (let i = 0; i < n; i++)
    out.push(spark(x, y, c, 18 + Math.random() * 16, 2 + Math.random() * 2));
  return out;
}
...
const WEAPONS: Record<WeaponId, { gap: number; sfx: () => void; onFire: FireFn }> = {
  blaster: {
    gap: 8,
    sfx: () => synth.shoot(),
    onFire: (p, ang) => [bullet(p.x + 18, p.y, 8, 4, Math.cos(ang) * 9, Math.sin(ang) * 9, "blaster")],
  },
  spread: {
    gap: 13,
    sfx: () => synth.spread(),
  ...
  piercer: {
    gap: 16,
    sfx: () => synth.pierce(),
    onFire: (p, ang) => [bullet(p.x + 20, p.y, 14, 5, Math.cos(ang) * 10, Math.sin(ang) * 10, "piercer", { pierce: 3 })],
  },
  laser: {
    gap: 22,
    sfx: () => synth.laser(),
    onFire: (p, ang) => [bullet(p.x + 26, p.y, 18, 6, Math.cos(ang) * 12, Math.sin(ang) * 12, "laser", { pierce: 6, dmg: 2 })],
  },
  rail: {
    gap: 32,
    sfx: () => synth.rail(),
    onFire: (p, ang) => [bullet(p.x + 24, p.y, 4, 72, Math.cos(ang) * 16, Math.sin(ang) * 16, "rail", { pierce: 8, dmg: 3 })],
  },
...
export default function Game({ lang, onProgress }: { lang: "he" | "en"; onProgress?: ProgressCb }) {
  /* --- core refs/state --- */
  ...
  // desktop inputs (Arrows + Space)
  const { fire, ax: kbAx, ay: kbAy } = useInput();
  ...
  // Phone thumbsticks:
  const { axRef: moveAxRef, ayRef: moveAyRef } = useThumbstick(canvasRef, MODE.current === "touch");
  const { axRef: aimAxRef, ayRef: aimAyRef, firingRef: fireRef } = useAimstick(
    canvasRef,
    MODE.current === "touch"
  );

  // Tap-to-move nudge (left half only, phone view)
  const tapTarget = useRef<null | { x: number; y: number; until: number }>(null);
  ...
  const player = useRef<PlayerState>({
    ...
    vx: 0,
    vy: 0,
    maxSpeed: MODE.current === "touch" ? 6.4 : 8.8,
    radius: BASE_R,
    hp: 6,
    shieldMs: 0,
    weapon: "blaster",
  ...
  const powerupsRef = useRef<
    Array<{ id: number; x: number; y: number; kind: AnyPU; payload?: WeaponId }>
  >([]);
  ...
  /* ===== phone: tap-to-move registration (left half) ===== */
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      const p = local(e);
      if (p.x <= c.clientWidth * 0.5) {
        tapTarget.current = { x: p.x, y: p.y, until: nowMs() + 350 };
      }
    };
  ...
  useEffect(() => {
    const c = canvasRef.current!,
      ctx = c.getContext("2d")!;
    c.style.touchAction = "none"; // iOS: no rubber banding, precise sticks
    ...
    const loop = () => {
      ...
      const tsBase = MODE.current === "touch" ? 0.92 : 1;
      ...
      /* input resolve (touch = left stick; desktop = arrows). Also tap-to-move nudge. */
      let axEff = MODE.current === "touch" ? moveAxRef.current || 0 : kbAx;
      let ayEff = MODE.current === "touch" ? moveAyRef.current || 0 : kbAy;

      if (MODE.current === "touch" && tapTarget.current && tNow < tapTarget.current.until) {
        const dx = tapTarget.current.x - player.current.x;
        const dy = tapTarget.current.y - player.current.y;
      ...
      const base = MODE.current === "touch" ? 0.14 : 0.22;
      ...
      player.current.x = clamp(
        ...
        PADDING + player.current.radius,
        W.current - PADDING - player.current.radius
      );
      player.current.y = clamp(
        player.current.y + player.current.vy,
        PADDING + player.current.radius,
        H.current - PADDING - player.current.radius
      );

      player.current.radius =
        lerp(player.current.radius, targetRadius.current, 0.2 * dt) * (1 + dmgBounce.current * 0.2);
      ...
      if (wantFire && frame.current - lastFire.current > effectiveGap * ts) {
        ...
        particlesRef.current.push(
          spark(
            player.current.x + Math.cos(aimAngle.current) * 20,
            player.current.y + Math.sin(aimAngle.current) * 20,
            "#a7f3d0",
            16,
            2
          )
      ...
      for (const b of bulletsRef.current)
        if (flag(b.data?.orb)) {
          const bd = ensureData(b);
          const ph = num(bd.phase, 0) + 0.14 * dt;
          bd.phase = ph;
          b.y = player.current.y + Math.sin(ph) * 26;
          b.x += 7 * dt;
        }

      /* spawns */
      const L = levelCfg.current;
      const spawnEvery = Math.max(14, Math.floor(L.spawnEvery * 1.2 * (hasteMs.current > 0 ? 0.85 : 1)));
      if (!bossActive && frame.current % spawnEvery === 0) {
        ...
        const drop = 0.82;
        if (Math.random() < drop) {
          const mythic = Math.random() < 0.2;
          if (mythic) {
            powerupsRef.current.push({
              id: id(),
              x: W.current + 50,
              y,
              kind: MYTHIC_POOL[Math.floor(Math.random() * MYTHIC_POOL.length)],
            });
          } else {
            const isBad = Math.random() < 0.08;
            const kindPU = (isBad ? BAD_ONLY : GOOD_POOL)[
              Math.floor(Math.random() * (isBad ? BAD_ONLY.length : GOOD_POOL.length))
            ];
            powerupsRef.current.push({
              id: id(),
              x: W.current + 50,
              y,
              kind: kindPU,
              payload: pickWeaponWeighted(stage),
            });
          }
        }
      }
      ...
      /* move bullets */
      for (const sp of bulletsRef.current) {
        sp.x += num(sp.vx, 0) * ts;
        sp.y += num(sp.vy, 0) * ts;
      }
      bulletsRef.current = bulletsRef.current.filter(
        (sp) => sp.x < W.current + 140 && sp.x > -140 && sp.y > -80 && sp.y < H.current + 80
      );
      ...
      for (const e of enemiesRef.current) {
        ...
        d.t = num(d.t, 0) + 0.03 * dt;
        d.blink = Math.sin(frame.current * 0.08 + e.id) > 0.96 ? 1 : 0;

        if (k === "jelly") {
          e.x += num(e.vx, 0) * 0.66 * ts;
          e.y += Math.sin(num(d.t) * 2.1) * 2.3;
        } else if (k === "squid") {
          e.x += num(e.vx, 0) * 0.98 * ts;
          e.y += Math.sin(num(d.t) * 3.0) * 1.5;
        } else if (k === "manta") {
          e.x += num(e.vx, 0) * 0.9 * ts;
          e.y += Math.sin(num(d.t) * 1.2) * 2.5;
        } else if (k === "nautilus") {
          e.x += num(e.vx, 0) * 0.82 * ts;
          ...
          e.x += num(e.vx, 0) * 0.92 * ts;
          d.scale = 1 + Math.sin(num(d.t) * 2.6) * 0.22;
        } else if (k === "crab") {
          e.x += num(e.vx, 0) * 0.94 * ts;
          e.y += Math.sin(num(d.t) * 2.2) * 1.0;
        } else if (k === "siren") {
          e.x += num(e.vx, 0) * 0.94 * ts;
          e.y += Math.sin(num(d.t) * 1.8) * 1.4;
          if (frame.current % 50 === 0) {
            ...
            enemiesRef.current.push({
              ...
              type: "enemy",
              hp: 1,
              data: { bullet: true, tint: "#60a5fa" },
            } as any);
          }
        }

        if (!flag(e.data?.bullet)) e.y += Math.sin(num(d.t) * 4 + e.id) * 0.28;
      }
      ...
      if (bossRef.current) {
        const b = bossRef.current,
          d = ensureData(b);
        b.x += -0.85 * ts;
        d.t = num(d.t, 0) + 0.02 * dt;
        d.aura = 0.8 + 0.2 * Math.sin(frame.current * 0.08);
        b.y = H.current / 2 + Math.sin(num(d.t)) * (68 + 10 * stage);
        if (frame.current % Math.max(18, 40 - stage * 2) === 0) {
          for (let i = -1; i <= 1; i++) {
            ...
            enemiesRef.current.push({
              ...
              w: 18,
              h: 6,
              vx: Math.cos(Math.PI + a) * (3.9 + stage * 0.11),
              vy: Math.sin(Math.PI + a) * 1.0,
              type: "enemy",
              hp: 1,
              data: { bullet: true, boss: "spear" },
            } as any);
          }
        }
        if (frame.current % 66 === 0) {
          ...
          for (let i = 0; i < n; i++) {
            ...
            enemiesRef.current.push({
              ...
              w: 10,
              h: 10,
              vx: Math.cos(ang) * (-3.0 - stage * 0.07),
              vy: Math.sin(ang) * 1.9,
              type: "enemy",
              hp: 1,
              data: { bullet: true, boss: "ring" },
            } as any);
          }
        }
        if (frame.current % 56 === 0) {
      ...
      for (const b of bulletsRef.current) {
        for (const e of enemiesRef.current) {
          if (flag(e.data?.bullet)) continue;
          if (aabb(b, e)) {
            const dmg = num(b.data?.dmg, 1);
            (e as any).hp = num(e.hp, 1) - dmg;
            particlesRef.current.push(
              ...explode(e.x, e.y, colorForEnemy(asEnemyKind(text(e.data?.kind, "jelly"))))
            );
            const bd = ensureData(b);
            let pierce = num(bd.pierce, 0);
            if (pierce > 0) bd.pierce = pierce - 1;
            else (b as any).x = W.current + 9999;
            if (num(e.hp, 0) <= 0) {
              if (!mute) synth.bonus();
        ...
        if (bossRef.current && aabb(b, bossRef.current)) {
          const dmg = num(b.data?.dmg, 1);
          (bossRef.current as any).hp = num(bossRef.current.hp, 1) - dmg;
          ...
          if (num(bossRef.current.hp, 0) <= 0) {
            ...
            for (let k = 0; k < 6; k++)
              powerupsRef.current.push({
                id: id(),
                x: W.current - 260 + k * 44,
                y: rnd(PADDING + 80, H.current - PADDING - 80),
                kind: Math.random() < 0.85 ? "weapon" : "drone",
                payload: pickWeaponWeighted(stage + 1),
              });
          }
        }
      }

      // friendly push on contact
      const pb = {
        x: player.current.x,
        y: player.current.y,
        w: player.current.radius * 2,
        h: player.current.radius * 2,
      } as any;
      for (const e of enemiesRef.current) {
        if (flag(e.data?.bullet)) continue;
        if (aabb(pb, e)) {
      ...
      for (const pu of powerupsRef.current) {
        ...
        if (aabb(pb, box as any)) {
          (pu as any).x = -9999;
          boostsCollected.current++;
          if (pu.kind === "shield") player.current.shieldMs = 3600;
          if (pu.kind === "speed") {
            player.current.maxSpeed = Math.min(
              MODE.current === "touch" ? 9.4 : 13.2,
              player.current.maxSpeed + (MODE.current === "touch" ? 0.5 : 0.9)
            );
            rapidMs.current = 5200;
          }
          if (pu.kind === "heal") {
            player.current.hp = Math.min(MAX_LIVES, player.current.hp + 1);
            recalcTargetRadius();
          }
          if (pu.kind === "weapon" && pu.payload) {
            const target = WEAPON_TIER.indexOf(asWeaponId(pu.payload));
            if (target > weaponLevel.current) weaponLevel.current = target;
            rapidMs.current = 5200;
          }
          ...
          if (frame.current % 26 === 0) {
            bulletsRef.current.push(
              bullet(
                ...
                7,
                7,
                Math.cos(aimAngle.current) * 8,
                Math.sin(aimAngle.current) * 8,
                "blaster"
              )
            );
            if (!mute) synth.shoot();
      ...
      for (const o of xpOrbsRef.current) {
        const dx = player.current.x - o.x,
          dy = player.current.y - o.y;
        const d = Math.max(0.001, Math.hypot(dx, dy));
        const pull = 0.06 * dt;
        o.vx += (dx / d) * pull;
        o.vy += (dy / d) * pull;
        ...
        if (d < 18) {
          xp.current += 4;
          o.life = 0;
          particlesRef.current.push(spark(player.current.x, player.current.y, "#fde047", 18, 3));
          if (!mute) synth.bonus();
        }
      }
      ...
      while (xp.current >= xpToNext.current) {
        ...
        for (let i = 0; i < 24; i++) {
          particlesRef.current.push({
            id: id(),
            x: player.current.x + rnd(-40, 40),
            y: player.current.y - 120 + rnd(-20, 20),
            w: 3,
            h: 3,
            vx: rnd(-1, 1),
            vy: rnd(1.2, 2.2),
            type: "spark",
            data: { life: 24, color: `hsl(${avatarHue.current},90%,60%)` },
          } as any);
        }
        if (!mute) synth.power();
      }

      // particles step + bubbles
      for (const sp of particlesRef.current) {
        const d = ensureData(sp);
        (sp as any).x += num((sp as any).vx, 0) * dt;
        (sp as any).y += num((sp as any).vy, 0) * dt;
        d.life = num(d.life, 0) - 1 * dt;
      }
      particlesRef.current = particlesRef.current.filter((sp) => num(sp.data?.life, 0) > 0);
      for (const bb of bubblesRef.current) {
        bb.y -= bb.v * dt;
        if (bb.y < -10) {
          bb.x = rnd(0, W.current);
          bb.y = H.current + rnd(10, 80);
          bb.r = rnd(2, 6);
          bb.v = rnd(0.35, 0.9);
        }
      }
  ...
  }, [paused, mute, stage, kbAx, kbAy, fire, gameOver]);

  const pickWeaponWeighted = (st: number): WeaponId => {
    const bag: WeaponId[] = ["blaster", "blaster", "spread", "piercer"];
    if (st >= 2) bag.push("spread", "piercer");
    if (st >= 3) bag.push("laser");
    if (st >= 4) bag.push("rail");
    if (st >= 5) bag.push("orbitals");
    if (st >= 6) bag.push("nova");
    return bag[Math.floor(Math.random() * bag.length)];
  };
  const pickEnemyKind = (st: number): EnemyKind => {
    const bag: EnemyKind[] = ["jelly", "squid", "manta", "puffer", "siren"];
    if (st >= 2) bag.push("nautilus");
    if (st >= 3) bag.push("crab");
    return bag[Math.floor(Math.random() * bag.length)];
  };
  ...
  const softReset = () => {
    ...
    player.current = {
      ...
      vx: 0,
      vy: 0,
      maxSpeed: MODE.current === "touch" ? 6.6 : 8.6,
      radius: BASE_R,
      hp: 6,
      shieldMs: 0,
      weapon: "blaster",
    };
    weaponLevel.current = 0;
    rapidMs.current = 0;
    hasteMs.current = 0;
    drones.current = [];
    ...
    powerupsRef.current = [];
    bossRef.current = null;
    levelCfg.current = makeLevel(stage);
    spawns.current = 0;
    hitFlash.current = 0;
    dmgBounce.current = 0;
    shake.current = 0;
    setScore(0);
    enemiesKilled.current = 0;
    boostsCollected.current = 0;
    xp.current = 0;
    xpToNext.current = 40;
    playerLevel.current = 1;
    avatarHue.current = 38;
    xpGainPulse.current = 0;
    barrierMs.current = 0;
    twinMs.current = 0;
    helpersMs.current = 0;
  ...
  function drawSeaDragon(ctx: CanvasRenderingContext2D, x: number, y: number, t: number) {
    ...
    roundCapsule(ctx, 10, -18, 44, 36, 12);
    ctx.fill();
    ctx.fillStyle = "#0b1220";
    ctx.beginPath();
    ctx.arc(46, -6, 3, 0, Math.PI * 2);
    ctx.arc(46, 6, 3, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  ...
  function draw(ctx: CanvasRenderingContext2D) {
    ...
    // light bands (fixed duplicated loop + braces)
    ...
    if (hitFlash.current > 0) {
      ctx.fillStyle = `rgba(239,68,68,${0.14 + 0.08 * Math.sin(frame.current * 0.5)})`;
      ctx.fillRect(0, 0, w, h);
    }
    if (hasteMs.current > 0) {
    ...
    if (shake.current > 0) {
      ctx.save();
      ctx.translate(rnd(-shake.current, shake.current), rnd(-shake.current, shake.current));
    }
    ...
    for (const pu of powerupsRef.current) {
      ...
      if (mythic) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(0, 0, 14 + Math.sin(frame.current * 0.16 + pu.id) * 2, 0, Math.PI * 2);
        ctx.stroke();
    ...
    for (const e of enemiesRef.current) {
      ...
      if (flag((e as any).data?.bullet)) {
        ...
        if (k === "jelly") {
          ...
          roundCapsule(ctx, -16, -12, 32, 26, 12);
          ctx.fill();
          ...
          roundCapsule(ctx, -28, -14, 56, 28, 8);
          ctx.fill();
          ctx.fillStyle = blueOn ? "#60a5fa" : "#ef4444";
          roundCapsule(ctx, -16, -16, 32, 10, 5);
          ctx.fill();
          ctx.fillStyle = blueOn ? "#ef4444" : "#60a5fa";
          roundCapsule(ctx, -16, 6, 32, 8, 5);
          ctx.fill();
          ctx.fillStyle = "#0b1220";
          ctx.beginPath();
    ...
    for (const b of bulletsRef.current) {
      ...
      const x = (b as any).x,
        ...
        w = (b as any).w,
        h = (b as any).h;
      const vx = num((b as any).vx, 8),
        vy = num((b as any).vy, 0);
      if (k === "rail") {
    ...
    const sx = p.radius * (1 + dmgBounce.current * 0.18),
      sy = p.radius * (1 - dmgBounce.current * 0.12);
    roundCapsule(ctx, p.x - sx * 0.45, p.y - sy * 0.3, sx * 0.9, sy * 1.2, 8);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(p.x, p.y - sy * 0.8, sx * 0.45, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (player.current.shieldMs > 0) {
      ...
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.radius + 10 + Math.sin(frame.current * 0.2) * 2, 0, Math.PI * 2);
      ctx.stroke();
    }
...
if (twinRef.current) {
  ...
  roundCapsule(ctx, twinRef.current.x - 10, twinRef.current.y - 8, 20, 30, 8);
  ctx.fill();
}
...
// XP bar
const xpW = Math.min(w - 160, 520),
  xpLeft = (w - xpW) / 2,
  xpTop = 12;
ctx.fillStyle = "rgba(0,0,0,.35)";
roundCapsule(ctx, xpLeft, xpTop, xpW, 12, 6);
ctx.fill();
...
const pad =
  Math.sin(frame.current * 0.2) * (xpGainPulse.current > 0 ? 4 * xpGainPulse.current : 0);
