/* Game.tsx — desktop: arrows/WASD + Space; phone: dual-stick (left=move, right=aim+fire) */
...
export default function Game({
  ...
  /* ========= dual-stick touch (no extra hooks) ========= */
  type Stick = {
    id: number | null;
    active: boolean;
    ox: number;
  ...
  const left = useRef<Stick>({ id: null, active: false, ox: 0, oy: 0, ax: 0, ay: 0, mag: 0 }).current;
  const right = useRef<Stick>({ id: null, active: false, ox: 0, oy: 0, ax: 0, ay: 0, mag: 0 }).current;
  const resetStick = (s: Stick) => {
    s.id = null;
    s.active = false;
    s.ax = s.ay = 0;
    s.mag = 0;
  ...
  const player = useRef<PlayerState>({
    ...
    vx: 0,
    vy: 0,
    maxSpeed: MODE.current === "touch" ? 6.6 : 9.0,
    radius: BASE_R,
    hp: 6,
    shieldMs: 0,
    weapon: "blaster",
  ...
  useEffect(() => {
    const c = canvasRef.current!,
      ctx = c.getContext("2d")!;
    c.style.touchAction = "none"; // allow precise sticks
    ...
    const loop = () => {
      ...
      const base = MODE.current === "touch" ? 0.18 : 0.22;
      ...
      /* === firing === */
      const wantFire =
        MODE.current === "touch" ? right.active && right.mag > 0.08 : !!kbFire;
      ...
      const spawnEvery = Math.max(
        14,
        Math.floor(L.spawnEvery * 1.2 * (hasteMs.current > 0 ? 0.85 : 1))
      );
      ...
      for (const sp of bulletsRef.current) {
        (sp as any).x += num((sp as any).vx, 0) * ts;
        (sp as any).y += num((sp as any).vy, 0) * ts;
      }
      ...
      for (const e of enemiesRef.current) {
        ...
        d.blink = Math.sin(frame.current * 0.08 + (e.id || 0)) > 0.96 ? 1 : 0;

        if (k === "jelly") {
          (e as any).x += num((e as any).vx, 0) * 0.66 * ts;
          (e as any).y += Math.sin(num(d.t) * 2.1) * 2.3;
        } else if (k === "squid") {
          (e as any).x += num((e as any).vx, 0) * 0.98 * ts;
          (e as any).y += Math.sin(num(d.t) * 3.0) * 1.5;
        } else if (k === "manta") {
          (e as any).x += num((e as any).vx, 0) * 0.9 * ts;
          (e as any).y += Math.sin(num(d.t) * 1.2) * 2.5;
        } else if (k === "nautilus") {
          (e as any).x += num((e as any).vx, 0) * 0.82 * ts;
          ...
          (e as any).x += num((e as any).vx, 0) * 0.92 * ts;
          (d as any).scale = 1 + Math.sin(num(d.t) * 2.6) * 0.22;
        } else if (k === "crab") {
          (e as any).x += num((e as any).vx, 0) * 0.94 * ts;
          (e as any).y += Math.sin(num(d.t) * 2.2) * 1.0;
        } else if (k === "siren") {
          (e as any).x += num((e as any).vx, 0) * 0.94 * ts;
          (e as any).y += Math.sin(num(d.t) * 1.8) * 1.4;
          if (frame.current % 50 === 0) {
            const speed = 3.6 + stage * 0.05;
            const dx = player.current.x - (e as any).x;
            const dy = player.current.y - (e as any).y;
      ...
      if (bossRef.current) {
        ...
        (d as any).aura = 0.8 + 0.2 * Math.sin(frame.current * 0.08);
      ...
      for (const e of enemiesRef.current)
        if (flag((e as any).data?.bullet)) {
          (e as any).x += num((e as any).vx, 0) * ts;
          (e as any).y += num((e as any).vy, 0) * ts;
        }
      ...
      for (const b of bulletsRef.current) {
        for (const e of enemiesRef.current) {
          ...
          if (aabb(b, e)) {
            const dmg = num((b as any).data?.dmg, 1);
            (e as any).hp = num((e as any).hp, 1) - dmg;
            particlesRef.current.push(
              ...explode((e as any).x, (e as any).y, colorForEnemy(asEnemyKind(text((e as any).data?.kind, "jelly"))))
            );
            const bd = ensureData(b);
        ...
        if (bossRef.current && aabb(b, bossRef.current)) {
          const dmg = num((b as any).data?.dmg, 1);
          (bossRef.current as any).hp = num((bossRef.current as any).hp, 1) - dmg;
      ...
      for (const e of enemiesRef.current) {
        if (flag((e as any).data?.bullet)) continue;
        if (aabb(pb, e)) {
          const dx = (e as any).x - player.current.x,
            dy = (e as any).y - player.current.y;
      ...
      if (helpersMs.current > 0) {
        ...
        for (const h of helpersRef.current) {
          h.phase += 0.06 * dt;
          (h as any).x = player.current.x + Math.cos(h.phase) * 50;
          (h as any).y = player.current.y + Math.sin(h.phase) * 50;
      ...
      if (twinMs.current > 0) {
        ...
        if (!twinRef.current)
          twinRef.current = { x: player.current.x + 24, y: player.current.y - 24, phase: 0 };
      ...
      for (const sp of particlesRef.current) {
        ...
        (d as any).life = num((d as any).life, 0) - 1 * dt;
      }
  ...
  function draw(ctx: CanvasRenderingContext2D) {
    ...
    ctx.textAlign = "right";
    ctx.fillText(`${Lbl.score} ${score}`, w - 12, 26);
    ctx.fillText(`${Lbl.stage} ${stage}${bossRef.current ? " • " + Lbl.boss : ""}`, w - 12, 48);
    ctx.fillText(`${Lbl.weapon} ${p.weapon.toUpperCase()}`, w - 12, 70);
